...
func New(baseURL, token string, client *http.Client) (*Client, error) {
	...
	return &Client{baseURL: strings.TrimRight(baseURL, "/"), token: token, serviceToken: strings.TrimSpace(os.Getenv("PDA_UPSTREAM_WMS_SERVICE_TOKEN")), http: client}, nil
}
...
func (c *Client) getJSON(ctx context.Context, path string, target any) error {
	...
	if err != nil {
		return fmt.Errorf("build WMS read request: %w", err)
	}
	c.applyHeaders(req, "")
	resp, err := c.http.Do(req)
	if err != nil {
		return fmt.Errorf("WMS read request: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		return statusError(resp)
