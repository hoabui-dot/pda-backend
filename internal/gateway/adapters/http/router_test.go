...
func setup(t *testing.T, limit int, logger *slog.Logger) http.Handler {
	...
	handler, err := New(service, taskService, nil, nil, nil, nil, nil, nil, nil, nil, Settings{RequestTimeout: 100 * time.Millisecond, AuthRateLimit: limit, RateWindow: time.Minute, CircuitFailureThreshold: 3}, logger, func() time.Time { return fixedTime })
	if err != nil {
		t.Fatal(err)
	}
	return handler
...
func TestInvalidGatewayResilienceSettingsRejected(t *testing.T) {
	if _, err := New(nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, Settings{}, slog.Default(), time.Now); err == nil {
